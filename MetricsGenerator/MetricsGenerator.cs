...
namespace MetricsGenerator
{
    ...
    public class MetricsGenerator: ISourceGenerator
    {
        ...
        private StringBuilder GetMetrics(List<MemberDeclarationSyntax> properties, string attrPrefix, Compilation compilation)
        {
            ...
            foreach (PropertyDeclarationSyntax p in properties)
            {
                ...
                if (metricAttr.HasParameterValues("labels") > 0)
                {
                    ...
                    text.AppendLine(
                        $"{{$\"{{prefix}}{attrPrefix}_{propName}\", Metrics.Create{metricType}($\"{{prefix}}{attrPrefix}_{propName}\", \"{propName}\", {labels}) }},");

                }
                else
                {
                    text.AppendLine(
                        $"{{$\"{{prefix}}{attrPrefix}_{propName}\", Metrics.Create{metricType}($\"{{prefix}}{attrPrefix}_{propName}\", \"{propName}\") }},");
                }
            }
            text.AppendLine(@"};
                            return result;
        ...
        {
            var text = new StringBuilder($"public static void UpdateMetrics(string prefix, MetricCollection metrics, {classToProcess} data, List<string> extraLabels = null) {{");
            text.AppendLine();
            ...
            foreach (PropertyDeclarationSyntax p in properties)
            {
                ...
                var newValue = metricAttr.GetParameterValue("valuePath") != null ? 
                    metricAttr.GetParameterValue("valuePath")
